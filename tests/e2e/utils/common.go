...
func HandleRule(operation string, apiserver string, UID string) (bool, int) {
	var req *http.Request
	var err error
	var body io.Reader
...
func SendMsg(url string) (bool, int) {
	var req *http.Request
	var err error
	...
	respBytes, err := json.Marshal("hello world.")
	if err != nil {
		Fatalf("Marshalling body failed: %v", err)
	}
	req, err = http.NewRequest(http.MethodPost, url, bytes.NewBuffer(respBytes))
	req.Header.Set("Content-Type", "application/json")
	if err != nil {
		// handle error
		Fatalf("Frame HTTP request failed: %v", err)
