...
var _ = Describe("Rule Management test in E2E scenario", func() {
	...
	Context("Test rule and rule-endpoint Creation and deletion", func() {
		...
		It("E2E_CREATE_RULE_1: Create rule: rest to eventbus.", func() {
			...
			IsRuleCreated, statusCode := utils.HandleRule(http.MethodPost, ctx.Cfg.K8SMasterForKubeEdge+RuleHandler, "")
			Expect(IsRuleCreated).Should(BeTrue())
			Expect(statusCode).Should(Equal(http.StatusCreated))
			newRule := utils.NewRule()
			_, err := utils.GetRuleList(&ruleList, ctx.Cfg.K8SMasterForKubeEdge+RuleHandler, newRule)
			Expect(err).To(BeNil())
			// call rest api to send message to edge.
			IsSend, statusCode := utils.SendMsg("http://127.0.0.1:9443/edge-node/default/ccc")
